...
스키마, 공통 금지사항)을 전제로 한다. 이 스킬은 TDD 방법론 고유
내용만 다룬다. 문서 작성 스타일은 단답형(개조식) — 단, 예시 코드·표는
원문 구조 유지.
...
## 11. 이 프로젝트 적용 사항 (기존 NGV tdd 스킬 반영, SWE.4 담당)
...
- Verify RED/GREEN 명령은 `python -m unittest` 계열 사용, 실제 실행
  ...
  실측 원칙과 동일).
- **양방향 추적성 — TPL-TRC-001 컬럼 소유**: 이 스킬은 `SWE.4` 컬럼을
  채운다(단위 테스트 케이스 ID 기재. 스키마·경로·공통 절차는
  `aspice-common` 4장 참조). `Coverage` 컬럼에는 함수/branch 커버리지
  실측 결과를 공유 기재한다(다른 스킬 기재분은 지우지 않는다).
