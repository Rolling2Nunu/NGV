...
3. `tdd` 절차(Red-Green-Refactor, 테스트 문서화, 뮤테이션 점검)를
   그대로 수행한다.
4. `coding-standards`의 품질 지표·네이밍·Doxygen 규칙을 그대로
   적용한다.
5. **실측 검증(필수)**: 코드 작성 후 오픈소스 도구로 품질 지표를 실제
   측정하고 결과를 보고한다. 측정 불가능하면 "실측 불가 — 환경 확인
   필요"로 명시하고, 임의로 기준 충족을 단정하지 않는다.
6. 외부 패키지 추가 시 SBOM/FOSS 라이선스 목록을 함께 갱신한다.
